...
rels = {
  ...
  firmware_build: "build/px4_sitl_#{'nolockstep'}",
...
def create_fcu_files(opts, m_num, abs, rels, contents, model) # opts[:gazebo_model]
  rc_file="rcS#{m_num}"
...
op = OptionParser.new do |op|
  ...
  op.on("--gazebo_model2 NAME", "gazebo second optional model name") { |p| opts[:gazebo_model2] = p }
...
opts[:n].times do |i|
  ...
  cd(abs[:workspace_firmware]) {
    ...
    cd(model_name) {
      rc_file = create_fcu_files(opts, m_num, abs, rels, contents, opts[:gazebo_model])
...
if opts.key?(:gazebo_model2) and opts.key?(:n2)
  opts[:n2].times do |i|
    ...
    @mav_port = opts[:base_port] + (opts[:n] + m_index)*opts[:port_step]
    @mav_port2 = @mav_port + 1
    ...
    model_name="#{opts[:gazebo_model2]}#{i+1}"
